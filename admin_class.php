...
Class Action {
	...
	function login(){
		// extract username and password from _POST and check from database
		extract($_POST);
		$result = $this->db->query("SELECT *,concat(firstname,' ',lastname) as name FROM users where email = '".$email."' and password = '".md5($password)."'  ");
		// login information correct, return 1
	...
	function save_project(){
		...
		$data = "";
		foreach($_POST as $k => $v){
			if(in_array($k, array('name','start_date','end_date','description'))){
				// if($k == 'description')
      ...
      else if(in_array($k, array('status', 'manager_id'))) {
				if(empty($data)){
					$data .= " $k=$v ";
				}else{
					$data .= ", $k=$v ";
		...
		//add new project: $id not exists
		if(empty($id)){
			$save = $this->db->query("INSERT INTO project_list set $data");
			if(isset($user_ids)) {
				// Get max project id
				$max_id = $this->db->query("SELECT MAX(id) as maxid from project_list")->fetch_assoc()['maxid'];
				foreach($user_ids as $user_id) {
					$this->db->query("INSERT INTO works_on (user_id, project_id) VALUE ($user_id, $max_id)");
				}
			}
		// update project: $id already exists
		} else {
			$save = $this->db->query("UPDATE project_list set $data where id = $id");
			foreach($user_ids as $user_id) {
				$this->db->query("INSERT INTO works_on (user_id, project_id) VALUE ($user_id, $id)");
			}
		}
		if($save){
			return 1;
